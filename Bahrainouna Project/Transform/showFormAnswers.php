        ...
        <?php
                if (isset($_GET["formSelector"]) && isset($_GET["submit"])){
                    ...
                    if (isset($theOwner)){
                        ...
                        echo "<label>List of Questions: </label><br>";
                        for ($i = 0; $i < $formReference->getQuestionSize(); $i++){
                            $questionReference = $formReference->getQuestion($i);
                            $questionReferer = new Question($questionReference->getQuestion());
                            echo $questionReferer->display();
                        }
        ?>
                            <div class="container">
                                <form action="showFormAnswers.php" method="get">
                                    <label for="questionSelector">Enter the question: </label>
                                    <input type="text" name="questionSelector" placeholder="The questions are displayed above for you to select">
                                    <br><input type="button" name="backer" value="Back">
                                    <input type="submit" name="submitter" value="Confirm">
                                </form>
                            </div>
        <?php
                            if (isset($_GET["questionSelector"]) && isset($_GET["submitter"])){
                                $formReference = new Form($_GET["formSelector"], $owner);
                                for ($a = 0; $a < $formReference->getQuestionSize(); $a++){
                                    $questionReference = $formReference->getQuestion($a);
                                    $questionReferer = new Question($_GET["questionSelector"]);
                                    if ($questionReference == $questionReferer){
                                        $questionReferer->setQuestion($questionReference->getQuestion());
                                        $questionReferer->setID($questionReference->getID());
                                        for ($b = 0; $b < $questionReferer->size(); $b++){
                                            $answerReference = new Answer($questionReference->getAnswer($b)->getKey(), $questionReference->getAnswer($b)->getDetail());
                                            echo $answerReference->getKey().". ".$answerReference->getDetail()."<br>";
                                        }
                                        echo "<br>";
                                    }
                                }
                            }
                            elseif (isset($_GET["backer"])){
